...
function loadDettaglioProgettoPab() {
...
function creaLinkDownloadRecuperoDaListaTipoDoc() {
    ...
    $('#DocumentiUDList tr td:nth-child(' + downloadIdx + ')').wrapInner(function () {
        ...
        var el = $("<a></a>").attr("href", "PAB.html?operation__downloadRecupero__" + str[0].split("=")[1]).addClass("DownloadFileSessione");
        return el;
    });
}
...
function creaLinkDownloadComponente() {   
    ...
    $('#CompDocList tr td:nth-child(' + downloadIdx + ')').wrapInner(function () {
        ...
        var el = $("<a></a>").attr("href", "PAB.html?operation__downloadComponente__" + idx).addClass("DownloadFileSessione");
        return el;
    });
}

function scriviTitoloPaginaDettaglio() {
    $('.contentTitle h2').html("Dettaglio della UD Documento di Gara");
}

function scriviTitoloPaginaDettaglioTipoDoc() {
    var tipoDoc = $('#Nm_tipo_doc_hidden').attr("value");
    $('.contentTitle h2').html("Dettaglio del tipo documento " + tipoDoc );
}