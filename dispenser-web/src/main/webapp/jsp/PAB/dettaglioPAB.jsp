...
<%@ page import="it.eng.dispenser.slite.gen.form.PABForm" %>
...
<sl:body>
	...
	<sl:content>
            ...
            <slf:lblField name="<%=PABForm.PabButtonList.SCARICA_PROGETTO%>" codiceOrganizzazione="${sessionScope['###_NOME_RICERCA']}" colSpan="2"/>
            <sl:newLine skipLine="true"/>
            <!--  piazzo le liste con i risultati -->
            <slf:section name="<%=PABForm.ListaDocumentiSection.NAME%>" styleClass="noborder w100">
                <slf:listNavBar name="<%=PABForm.DocumentiUDList.NAME%>" pageSizeRelated="true"/>
                <slf:list name="<%=PABForm.DocumentiUDList.NAME%>" codiceOrganizzazione="${sessionScope['###_NOME_RICERCA']}" />
                <slf:listNavBar name="<%=PABForm.DocumentiUDList.NAME%>" />
            </slf:section>
            <sl:newLine skipLine="true"/>
	</sl:content>
	<sl:footer />
