...
<%@ page import="it.eng.dispenser.slite.gen.form.LUMForm" pageEncoding="UTF-8"%>
...
<sl:html>
    ...
    <sl:body>
        ...
        <sl:content>
            ...
            <slf:fieldBarDetailTag name="<%= LUMForm.Back.NAME%>" />  
            <h2>Referti collegati al referto ID <%= ((LUMForm)pageContext.getSession().getAttribute("###_FORM_CONTAINER")).getRefertiCollegatiList().getTable().getRow(0).getString("cdud") %> del <%= ((LUMForm)pageContext.getSession().getAttribute("###_FORM_CONTAINER")).getRefertiCollegatiList().getTable().getRow(0).getString("dataRefertoString") %></h2>
            <sl:newLine skipLine="true"/>
            <slf:list name="<%=LUMForm.RefertiCollegatiList.NAME%>" />
        </sl:content>
        <sl:footer />
