...
public class PUGAction extends PUGAbstractAction {
        ...
        try {
            if (getNavigationEvent().equals(NE_DETTAGLIO_VIEW)
                ...
                if (getTableName().equals(getForm().getStrumentiCollegatiList().getName())) {
                    ...
                    AroVLisDocRowBean currentRow = (AroVLisDocRowBean) getForm()
                            .getDocumentiUDList().getTable().getCurrentRow();
                    ...
                    AroVLisCompDocTableBean compDoc = ricercaBo.getAroVLisCompDocTableBean(idUd,
                            nmTipoDoc);
                    ...
                    try {
                        ...
                        // Prendendo i dati dalla form, popolo un oggetto List generico del tipo
                        ...
                        // Se nullo il relativo parametro, siamo per forza al primo livello di
                        // annidamento
                        BigDecimal level = getForm().getLevel().getLevel_strumento_urb()
                                .parse() != null
                                        ? getForm().getLevel().getLevel_strumento_urb().parse()
                                        : BigDecimal.ONE;
                        // Carico il dettaglio strumento urbanistico
                        BigDecimal idUnitaDoc = (listRow
                                .getBigDecimal("id_unita_doc_colleg") != null)
                                        ? listRow.getBigDecimal("id_unita_doc_colleg") // Sto
                                        // gestendo
                                        // la lista
                                        // dei
                                        // collegamenti
                                        : listRow.getBigDecimal("id_unita_doc"); // Sto gestendo la
                        // lista di ricerca
                        // al primo
                        // livello
                        BaseRow row = ricercaBo.caricaDettaglioUd(idUnitaDoc,
                                ...
                                DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                                        .getCurrentRow().getBigDecimal("id_strut").longValue());
                        DynamicSpagoLiteForm.getFormDettaglio(getForm()).copyFromBean(row);
                        //
                        AroVLisDocTableBean listDocUd = ricercaBo.getAroVLisDocTableBean(
                                (BigDecimal) row.getBigDecimal("id_unita_doc"));
                        ...
                        AroVLisLinkUnitaDocTableBean linkUd = ricercaBo
                                .getAroVLisLinkUnitaDocTableBean(row.getBigDecimal("id_unita_doc"),
                                        getRicercaSelezionata());
                        int linkUDPageSize = 10;
                        if (getForm().getStrumentiCollegatiList().getTable() != null) {
                            linkUDPageSize = getForm().getStrumentiCollegatiList().getTable()
                                    .getPageSize();
                        }
                        getForm().getStrumentiCollegatiList().setTable(linkUd);
                        getForm().getStrumentiCollegatiList().getTable()
                                .setPageSize(linkUDPageSize);
                        ...
                        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
                            ...
                            // Inserisco nello stack di navigazione i dati del nuovo strumento
                            // urbanistico che sto per
                            // visualizzare:
                            // 1) id unita doc
                            ...
                            NavigatorBeanManager.getNavigatorBeanManager(getSession())
                                    .pushNavigatorBeanStack(idUnitaDoc,
                                            listRow.getBigDecimal("id_strut"), formList.getName(),
                                            formList.getTable(),
                                            formList.getTable().getCurrentRowIndex(),
                                            formList.getTable().getPageSize(), getForm().getLevel()
                                                    .getLevel_strumento_urb().parse().intValue());
                        }
                    } catch (SQLException e) {
                        throw new EMFError("Errore SQL: ", e);
                    }
            ...
            if (publisherName.equals(Application.Publisher.DETTAGLIO_PUG)) {
                ...
                if (getLastPublisher().equals(publisherName)) {
                    ...
                    if (idUnitaDoc != null && idStrut != null) {
                        ...
                        if (idUnitaDoc == null) {
                            idUnitaDoc = (DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                                    .getCurrentRow().getBigDecimal("id_unita_doc_colleg") != null)
                                            ? DynamicSpagoLiteForm.getRicercaList(getForm())
                                                    .getTable().getCurrentRow()
                                                    .getBigDecimal("id_unita_doc_colleg") // Sto
                                            // gestendo
                                            // la
                                            // lista
                                            // dei
                                            ...
                                            : DynamicSpagoLiteForm.getRicercaList(getForm())
                                                    .getTable().getCurrentRow()
                                                    .getBigDecimal("id_unita_doc"); // Sto gestendo
                            // la lista di
                            // ricerca
                            // al primo
                            // livello
                        ...
                        if (idStrut == null) {
                            idStrut = DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                    ...
                    if (detailBean != null) {
                        ...
                        DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                                .setPageSize(detailBean.getPageSize());
                    ...
                    idUnitaDoc = (DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                            .getCurrentRow().getBigDecimal("id_unita_doc_colleg") != null)
                                    ? DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                                            .getCurrentRow().getBigDecimal("id_unita_doc_colleg") // Sto
                                    // gestendo
                                    // la
                                    // lista
                                    // dei
                                    // collegamenti
                                    : DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                                            .getCurrentRow().getBigDecimal("id_unita_doc"); // Sto
                    // gestendo
                    // la
                    // lista
                    // di
    ...
    @SuppressWarnings({
            "unchecked", "rawtypes" })
    public void loadDettaglioConListe(String... rigaElemento)
            throws EMFError, ParseException, SQLException {
        // Carico il dettaglio dello strumento urbanistico
        BaseRow row = null;
        int index = 0;
        if (rigaElemento != null && rigaElemento.length > 0) {
            index = Integer.parseInt(rigaElemento[0]);
            row = (BaseRow) DynamicSpagoLiteForm.getRicercaList(getForm()).getTable().getRow(index);
        } else {
        ...
        BaseRow detailRow = ricercaBo.caricaDettaglioUd(idUnitaDoc, getRicercaSelezionata(),
                idStrut.longValue());
        form.getFormDettaglio().copyFromBean(detailRow);

        /*
         * Attenzione: ora devo dire al framework che ora mi trovo in PugForm nel quale setto anche
         * tutti i componenti di DynamicSpagoLiteForm, per mantenere il comportamento dinamico del
         * dettaglio
         */
        ...
        ((it.eng.spagoLite.form.list.List<SingleValueField<?>>) pug
                .getComponent(form.getRicercaList().getName()))
                .setTable(DynamicSpagoLiteForm.getRicercaList(getForm()).getTable());
        ((it.eng.spagoLite.form.list.List<SingleValueField<?>>) pug
                .getComponent(form.getRicercaList().getName())).getTable()
                .setCurrentRowIndex(index);
        setForm(pug);

        // Al primo giro azzero lo stack
        NavigatorBeanManager.getNavigatorBeanManager(getSession()).resetNavigatorBeanStack();
        // Salvo in sessione il "tipo" di lista dalla quale "proviene" lo strumento urbanistico,
        // sarà RicercaList
        String listName = DynamicSpagoLiteForm.getRicercaList(getForm()).getName();
        ...
        AroVLisDocTableBean listDocUd = ricercaBo
                .getAroVLisDocTableBean((BigDecimal) row.getObject("id_unita_doc"));
        getForm().getDocumentiUDList().setTable(listDocUd);
        ...
        AroVLisLinkUnitaDocTableBean linkUd = ricercaBo.getAroVLisLinkUnitaDocTableBean(
                (BigDecimal) row.getObject("id_unita_doc"), getRicercaSelezionata());
        getForm().getStrumentiCollegatiList().setTable(linkUd);
        ...
        NavigatorBeanManager.getNavigatorBeanManager(getSession()).pushNavigatorBeanStack(
                listRow.getBigDecimal("id_unita_doc"), listRow.getBigDecimal("id_strut"),
                formList.getName(), formList.getTable(), formList.getTable().getCurrentRowIndex(),
                formList.getTable().getPageSize(), 1);
    ...
    public void loadDettaglioCollConListe() throws SQLException, EMFError {
        ...
        getSession().setAttribute("navTableStrumentoUrbanistico",
                getForm().getStrumentiCollegatiList().getName());
        // Prendendo i dati dalla form, popolo un oggetto List generico del tipo gestito dal
        // framework naturalmente
        it.eng.spagoLite.form.list.List<?> formList = ((it.eng.spagoLite.form.list.List) getForm()
        ...
        formList.getTable().setCurrentRowIndex(
                getForm().getStrumentiCollegatiList().getTable().getCurrentRowIndex());
        formList.getTable()
                .setPageSize(getForm().getStrumentiCollegatiList().getTable().getPageSize());
        ...
        if (currentRow.getFlRisolto().equals("1")) {
            ...
            if (ListAction.NE_NEXT.equals(getNavigationEvent())
                    || ListAction.NE_PREV.equals(getNavigationEvent())) {
                // Se l'ultimo record di navigazione è presente e il livello di annidamento è lo
                // stesso
                if (NavigatorBeanManager.getNavigatorBeanManager(getSession())
                        .getLastNavigatorBeanStack() != null
                        && level.intValue() == NavigatorBeanManager
                                .getNavigatorBeanManager(getSession()).getLastNavigatorBeanStack()
                                .getLevel()) {
                    // Estraggo il record di navigazione
                    NavigatorBeanManager.getNavigatorBeanManager(getSession())
                            .popNavigatorBeanStack();
            ...
            } // Se invece sto andando in un dettaglio di un ente, sto andando ad un successivo
              // livello di profondità
            else {
                int nextLevel = NavigatorBeanManager.getNavigatorBeanManager(getSession())
                        .getLastNavigatorBeanStack().getLevel() + 1;
            ...
            // Ora che ho "gestito" pocanzi i livelli di annidamento, mi concentro sul caricamento
            // del dettaglio vero e
            // proprio
            if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
                    || getNavigationEvent().equals(ListAction.NE_NEXT)
                    || getNavigationEvent().equals(ListAction.NE_PREV)) {
                // Inserisco nello stack di navigazione i dati del nuovo strumento urbanistico
                // collegato che sto per
                // visualizzare:
                // 1) id unita doc (collegato)
                // 2) nome lista di ricerca dinamica contenente la lista dei collegamenti
                // 3) Tabella della ricerca dinamica conentente la tabella della lista dei
                // collegamenti
                // 5) Indice di riga della lista dei collegamenti
                // 6) Page size della lista dei collegamenti
                // 7) Livello di annidamento
                NavigatorBeanManager.getNavigatorBeanManager(getSession()).pushNavigatorBeanStack(
                        row.getBigDecimal("id_unita_doc"), row.getBigDecimal("id_strut"),
                        formList.getName(), formList.getTable(),
                        formList.getTable().getCurrentRowIndex(), formList.getTable().getPageSize(),
                        getForm().getLevel().getLevel_strumento_urb().parse().intValue());
            }
        } else {
    ...
    public BaseRow loadDettaglioConListe(BigDecimal idUnitaDoc, BigDecimal idStrut)
    ...
    public void downloadRecupero(String... rigaElemento) throws EMFError {
        ...
        String urlParam = applicationProperties
                .getProperty(ConstDipParamApplic.NmParamApplic.URL_RECUP_UD.name());
        Integer timeoutParam = Integer.parseInt(applicationProperties
                .getProperty(ConstDipParamApplic.NmParamApplic.TIMEOUT_RECUP_UD.name()));
        ...
        String pwdParam = applicationProperties
                .getProperty(ConstDipParamApplic.NmParamApplic.PSW_RECUP_UD.name());
        ...
        BigDecimal idStrut = DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                .getCurrentRow().getBigDecimal("id_strut");
        ...
        String filename = "UD_" + strut.getRegistro() + "_" + strut.getAnno().toPlainString() + "_"
                + strut.getNumero() + "_" + strut.getTipoDocumento() + ".zip";
        callRecupero(TipoRichiesta.REC_DIP_UNI_DOC, urlParam, timeoutParam, strut, versioneParam,
                userIdParam, pwdParam, filename);
    }
    ...
    public void downloadComponente(String... rigaElemento) throws EMFError {
        ...
        // String filename = "COMP_" + strut.getRegistro() + "_" + strut.getAnno().toPlainString() +
        // "_"
        // + strut.getNumero() + ".zip";
        String filename = row.getDsUrnCompCalc().replace("urn:", "").replaceAll(":", "_") + ".zip";
        callRecupero(TipoRichiesta.REC_COMP, urlParam, timeoutParam, strut, versioneParam,
                userIdParam, pwdParam, filename);
    }

    private void callRecupero(TipoRichiesta tipoRichiesta, String urlParam, Integer timeoutParam,
            RecuperoWSBean strut, String versioneParam, String userIdParam, String pwdParam,
            String filename) throws EMFError {
        String xmlRequest = "";
        try {
            ...
            recXml.getChiave().setOrdinePresentazioneComponente(
                    strut.getOrdinePresentazioneComponente() != null
                            ? strut.getOrdinePresentazioneComponente().intValue()
                            : null);
        ...
        if (!getMessageBox().hasError()) {
            RichiestaWSInput input = new RichiestaWSInput(tipoRichiesta, urlParam, timeoutParam,
                    true,
                    new BasicNameValuePair(RichiestaWSInput.CampiRichiesta.VERSIONE.name(),
                            versioneParam),
                    new BasicNameValuePair(RichiestaWSInput.CampiRichiesta.LOGINNAME.name(),
                            userIdParam),
                    new BasicNameValuePair(RichiestaWSInput.CampiRichiesta.PASSWORD.name(),
                            pwdParam),
                    new BasicNameValuePair(RichiestaWSInput.CampiRichiesta.XMLSIP.name(),
                            xmlRequest));

            EsitoConnessione esito = richiestaWSClient.callWs(input);
            if (!esito.isErroreConnessione()
                    && esito.getCodiceEsito().equals(EsitoConnessione.Esito.OK.name())) {
                /*
                ...
                getMessageBox().addError("Errore dal servizio di recupero: "
                        + (esito.isErroreConnessione() ? esito.getDescrErrConnessione()
                                : esito.getMessaggioErrore()));
            }
        }
    }

    /*
     * Ridefinisco questo metodo in modo che le superclassi sappiano che la Action è relativa a
     * delle pagine gestite dinamicamente con una jsp sola e quindi anteporranno alla normale
     * destination (es. /ricerca/ricerca) il suffisso ["nomeOrganizzazione"] dove il contenuto di
     * nomeorganizzazione viene restituito da questo metodo.
     */
    @Override
    ...
    public void scaricaStrumento() throws Throwable {
        ...
        BaseRow row = (BaseRow) DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                .getCurrentRow();
        BigDecimal idUnitaDoc = (row.getBigDecimal("id_unita_doc_colleg") != null)
                ...
                : row.getBigDecimal("id_unita_doc"); // Sto gestendo la lista di ricerca al primo
        // livello
