...
import it.eng.dispenser.slite.gen.action.LUMAbstractAction;
import it.eng.dispenser.slite.gen.form.LUMForm;
...
public class LUMAction extends LUMAbstractAction {
    ...
    private org.slf4j.Logger log = LoggerFactory.getLogger(LUMAction.class);
    ...
    protected String getDefaultPublsherName() {
        return Application.Publisher.REFERTI_COLLEGATI_LUM;
    }
    ...
    public String getControllerName() {
        return Application.Actions.LUM;
    }

    public void refCollegati(String... rigaElemento) {
        ...
        /*
         * Attenzione: ora devo dire al framework che ora mi trovo in LUMForm
         */
        LUMForm lum = new LUMForm();
        setForm(lum);
        ...
        AroVLisLinkUnitaDocTableBean linkUd = ricercaBo.getAroVLisLinkUnitaDocTableBean(
                (BigDecimal) row.getObject("id_unita_doc"), cdUd, dataRefertoString);
        ...
        forwardToPublisher(Application.Publisher.REFERTI_COLLEGATI_LUM);
    }

    public void refCollegati() {
        refCollegati((String[]) null);
    }

    public void loadRefertoDisponibile() throws SQLException, EMFError {
        // Recupero il tipo comp doc
        BigDecimal idUnitaDoc = ((AroVLisLinkUnitaDocTableBean) getForm().getRefertiCollegatiList()
                .getTable()).getCurrentRow().getIdUnitaDocColleg();
        BigDecimal idStrut = ((AroVLisLinkUnitaDocTableBean) getForm().getRefertiCollegatiList()
                .getTable()).getCurrentRow().getIdStrut();
        redirectToDettaglioPage(idUnitaDoc, idStrut);
    }

    public void redirectToDettaglioPage(BigDecimal idUnitaDoc, BigDecimal idStrut)
        ...
        redirectToPage(Application.Actions.RICERCA, form, form.getRicercaList().getName(), table,
                getNavigationEvent());
    }
    ...
    public void downloadComponente(String... rigaElemento) throws EMFError {
        ...
        if ((tmp = getForm()) instanceof DynamicSpagoLiteForm) {
            getSession().setAttribute(Constants.SESSIONE_FORM_RIC, tmp);
            form = tmp;
        }
        if (rigaElemento != null) {
    ...
    public void downloadPdf(String... rigaElemento) throws EMFError {
        ...
        String filename = "COMP_TRASF_" + strut.getRegistro() + "_"
                + strut.getAnno().toPlainString() + "_" + strut.getNumero() + ".zip";
        callRecupero(TipoRichiesta.REC_PDF, urlParam, timeoutParam, strut, versioneParam,
                userIdParam, pwdParam, filename);
    }
    ...
    public void downloadAip(String... rigaElemento) throws EMFError {
        ...
        String urlParam = applicationProperties
                .getProperty(ConstDipParamApplic.NmParamApplic.URL_RECUP_AIP.name());
        Integer timeoutParam = Integer.parseInt(applicationProperties
                .getProperty(ConstDipParamApplic.NmParamApplic.TIMEOUT_RECUP_UD.name()));
        String versioneParam = applicationProperties
                .getProperty(ConstDipParamApplic.NmParamApplic.VERSIONE_XML_RECUP_UD.name());
        ...
        RecuperoWSBean strut = pievesestinaBO.getUdData(row.getBigDecimal("id_strut"),
                row.getBigDecimal("id_unita_doc"));
        String filename = "AIP_" + strut.getRegistro() + "_" + strut.getAnno().toPlainString() + "_"
                + strut.getNumero() + ".zip";
        callRecupero(TipoRichiesta.REC_AIP, urlParam, timeoutParam, strut, versioneParam,
                userIdParam, pwdParam, filename);
    }
