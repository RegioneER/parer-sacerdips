...
public class RicercaAction extends RicercaAbstractAction {
	...
	if (p != null && p.getChild(menuEntry) != null) {
	    ...
	    LOG.debug("Utente " + getUser().getUsername()
		    + " non autorizzato all'esecuzione del metodo " + menuEntry + " in pagina "
		    + getLastPublisher());
	    getMessageBox().addFatal("Utente " + getUser().getUsername()
		    + " non autorizzato all'esecuzione dell'azione " + menuEntry + " in pagina "
		    + getLastPublisher());
	    return false;
	}
	...
	if (getNavigationEvent().equals(NE_DETTAGLIO_VIEW) || getNavigationEvent().equals(NE_NEXT)
		...
		BaseRow row = ricercaBo.caricaDettaglioUd(
			getForm().getRicercaList().getTable().getCurrentRow()
				.getBigDecimal("id_unita_doc"),
			getRicercaSelezionata(), getForm().getRicercaList().getTable()
