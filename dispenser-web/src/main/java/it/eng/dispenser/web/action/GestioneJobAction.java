...
public class GestioneJobAction extends GestioneJobAbstractAction {
	...
	if (filtri.validate(getMessageBox())) {
		...
		getForm().getJobSchedulatiList()
			.setTable(jobBo.getMonVLisSchedJobViewBean(filtri, dateValidate));
	...
	getForm().getFiltriJobSchedulati().getNm_job()
		.setDecodeMap(ComboGetter.getMappaSortedGenericEnum(COMBO_KEY_NM_JOB,
			Constants.NomiJob.getComboSchedulazioniJob()));
	...
	redirectToPage(Application.Actions.GESTIONE_JOB, form,
	...
	DecodeMap dec = ComboGetter.getMappaSortedGenericEnum(COMBO_KEY_NM_JOB,
		Constants.NomiJob.values());
	...
	getForm().getGestioneJobRicercaFiltri().getTi_stato_job()
		.setDecodeMap(getMappaTiStatoJob());
    }
	...
	if (tabella != null) {
		...
		if (jobSchedulatiString.length() > 1) {
		    message.append("Sono stati schedulati i seguenti job: <br><br>")
	...
	if (tabella != null) {
		...
		if (jobSchedulatiString.length() > 1) {
		    message.append("Sono stati stoppati i seguenti job: <br><br>")
			    .append(jobSchedulatiString).append("<br>");
		}
		...
		if (jobNoTimerString.length() > 1) {
		    message.append("<br>Non sono stati stoppati i seguenti job: <br><br>")
			    .append(jobNoTimerString).append("<br> in quanto di tipo NO_TIMER<br>");
		}
		message.append(
			"<br>L'operazione richiesta diventerà effettiva entro il prossimo minuto.");
		getMessageBox().clear();
	...
	if (tabella != null) {
		...
		if (jobSchedulatiString.length() > 1) {
		    message.append(
			    "Sono stati attivati in esecuzione singola i seguenti job: <br><br>")
			    .append(jobSchedulatiString).append("<br>");
		}
	...
	getForm().getGestioneJobInfo().getDisabilitaAllJobs()
		.setReadonly(!abilitaDisabilitaAllJobs);
    }
