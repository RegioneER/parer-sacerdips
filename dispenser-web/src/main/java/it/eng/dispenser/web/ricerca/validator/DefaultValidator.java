...
public class DefaultValidator extends IValidator {
    ...
    public boolean validate(Fields<Field> fields, MessageBox messageBox) {
        ...
        HashMap<String, IntervalDate> intervals = new HashMap<String, IntervalDate>(); // Used to
        // store the
        // intervals
        // extremes
        ...
        for (Field field : fields) {
            ...
            if (typeField.equals(Type.STRING)) {
                        ...
                        if (value.length() <= CHAR_BEFORE) {
                            messageBox.addMessage(new Message(Message.MessageLevel.ERR,
                                    "Nel campo \"" + field.getDescription()
                                            + "\" prima del carattere '%' ci devono essere "
                                            + CHAR_BEFORE + " caratteri"));
                            return false;
                        }
                    }
                    /*
                     * tutti i campi stringa li converte in uppercase per effettuare la ricerca su
                     * dati del db che sono già stati convertiti in uppercase. In tal modo le
                     * ricerche funzionano anche se l'utente inserisce un dato in minuscolo.
                     */
                ...
                // Used to test prompt
                log.debug("String field \"" + field.getName() + "\" (\"" + field.getDescription()
                        + "\"): " + value);
                ...
                if (value != null) {
                    ...
                    if (name.endsWith(SUFFIX_DATA_START)) {
                        name = name.subSequence(0, name.length() - SUFFIX_DATA_START.length())
                                .toString();
                        ...
                        if (intervals.containsKey(name)) {
                            ...
                            if (value.after(date_after.getDate())) {
                                messageBox.addMessage(new Message(Message.MessageLevel.ERR,
                                        "Il campo \"" + field.getDescription()
                                                + "\" e' successivo a \""
                                                + date_after.getDescription() + "\""));
                                return false;
                            } else {
                            ...
                            IntervalDate interval = new IntervalDate(true,
                                    dateField.getDescription(), value);
                            intervals.put(name, interval);
                        }
                    } else if (dateField.getName().endsWith(SUFFIX_DATA_END)) {
                        name = name.subSequence(0, name.length() - SUFFIX_DATA_END.length())
                                .toString();
                        ...
                        if (intervals.containsKey(name)) {
                            ...
                            if (str.trim().equals("")) {
                                ...
                                IntervalDate interval = new IntervalDate(false,
                                        dateField.getDescription(), value);
                                intervals.put(name, interval);
                            }
                        }
                ...
                // Used to test prompt
                log.debug("Date field \"" + field.getName() + "\" (\"" + field.getDescription()
                        + "\"): " + value);
            } else if (!one) {
                if (field instanceof SingleValueField) {
                    ...
                    log.debug(multiField.getType().toString() + " field \"" + multiField.getName()
                            + "\" (\"" + multiField.getDescription() + "\"): "
                            + multiField.getValues());
                }
            }
        }
        ...
        if (!one) {
            messageBox
                    .addMessage(new Message(Message.MessageLevel.ERR, "Inserire almeno un campo"));
            return false;
        } else if (!intervals.isEmpty()) {
            // If only one of date interval is set throws an error message.
            for (IntervalDate i : intervals.values()) {
                messageBox.addMessage(
                        new Message(Message.MessageLevel.ERR, "Hai inserito solo il campo \""
                                + i.getDescription() + "\" dell'intervallo temporale"));
            }
            return false;
        }
