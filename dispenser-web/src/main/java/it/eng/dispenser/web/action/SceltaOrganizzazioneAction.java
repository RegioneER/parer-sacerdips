...
public class SceltaOrganizzazioneAction extends ActionBase {
	...
	if (utente.isUtenteDaAssociare()) {
		...
		getMessageBox().addError(
			"Errore durante il recupero delle autorizzazioni: " + ex.getMessage());
	    }
	    redirectToAction(
		    it.eng.dispenser.slite.gen.Application.Actions.HOME + "?clearhistory=true");
	}
    }
	 ...
	 * c'̬ ancora perch̩ deve essere ancora associato Quindi si prende il suo codice fiscale
	 * se presente, altrimenti una stringa fissa come username
	 */
	...
	sacerLogHelper.insertEventoLoginUser(username, getIpClient(), new Date(),
		...
		"SACERDIPS - " + ConstLogEventoLoginUser.DS_EVENTO_BAD_CF_SPID, utente.getCognome(),
		utente.getNome(), utente.getCodiceFiscale(), utente.getExternalId(),
		utente.getEmail());
	String retURL = applicationProperties.getProperty(
		ConstDipParamApplic.NmParamApplic.URL_BACK_ASSOCIAZIONE_UTENTE_CF.name());
	...
	try {
	    this.getResponse()
		    .sendRedirect(applicationProperties.getProperty(
			    ConstDipParamApplic.NmParamApplic.URL_ASSOCIAZIONE_UTENTE_CF.name())
			    + "?r=" + Base64.encodeBase64URLSafeString(retURL.getBytes()) + "&h="
			    + hmac + "&s=" + salt + "&f=" + f + "&c=" + c + "&n=" + n);
	...
	if (user.getCodiceFiscale() != null && !user.getCodiceFiscale().isEmpty()) {
		...
		getMessageBox().addInfo(
			"L'utente loggato ̬ stato ricondotto con successo all'utenza Parer.");
		return;
	    }
	}
	/*
