...
public class SISMAAction extends SISMAAbstractAction {
            ...
            if (getNavigationEvent().equals(NE_DETTAGLIO_VIEW)
                ...
                if (getTableName().equals(getForm().getProgettiCollegatiList().getName())) {
                    ...
                    try {
                        ...
                        // annidamento
                        BigDecimal level = getForm().getLevel().getLevel_progetto_ric()
                                .parse() != null
                                        ? getForm().getLevel().getLevel_progetto_ric().parse()
                                        : BigDecimal.ONE;
                        ...
                        BaseRow row = ricercaBo.caricaDettaglioUd(idUnitaDoc,
                                ...
                                DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                                        .getCurrentRow().getBigDecimal(ID_STRUT_NAME).longValue());
                        ...
                        if (ListAction.NE_NEXT.equals(getNavigationEvent())
                            ...
                            if (NavigatorBeanManager.getNavigatorBeanManager(getSession())
                                ...
                                getForm().getLevel().getLevel_progetto_ric()
            ...
            if (publisherName.equals(Application.Publisher.DETTAGLIO_SISMA)) {
                ...
                if (getLastPublisher().equals(publisherName)) {
                    ...
                    idUnitaDoc = DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                            .getCurrentRow().getBigDecimal(ID_UNITA_DOC_NAME); // Sto gestendo la
                    // lista di ricerca
                    // al primo
                    idStrut = DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                            .getCurrentRow().getBigDecimal(ID_STRUT_NAME);
                }
    ...
    public void loadDettaglioCollConListe() throws SQLException, EMFError {
        ...
        getSession().setAttribute("navTableProgettoRicostruzione",
                getForm().getProgettiCollegatiList().getName());
        ...
        formList.getTable().setCurrentRowIndex(
                getForm().getProgettiCollegatiList().getTable().getCurrentRowIndex());
        formList.getTable()
                .setPageSize(getForm().getProgettiCollegatiList().getTable().getPageSize());
        ...
        BaseRow row = loadDettaglioConListe(idUnitaDoc, idStrut,
                getForm().getProgettiCollegatiList().getTable().getCurrentRow()
                        .getString(CODICE_INTERVENTO_NAME));
        // Se sto scorrendo orizzontalmente da un record all'altro
        if (ListAction.NE_NEXT.equals(getNavigationEvent())
                || ListAction.NE_PREV.equals(getNavigationEvent())) {
        ...
        } // Se invece sto andando in un dettaglio di un ente, sto andando ad un successivo livello
          // di profondità
        else {
            int nextLevel = NavigatorBeanManager.getNavigatorBeanManager(getSession())
                    .getLastNavigatorBeanStack().getLevel() + 1;
        ...
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
                || getNavigationEvent().equals(ListAction.NE_NEXT)
                || getNavigationEvent().equals(ListAction.NE_PREV)) {
            // Inserisco nello stack di navigazione i dati del nuovo strumento urbanistico collegato
            // che sto per
            // visualizzare:
            ...
            NavigatorBeanManager.getNavigatorBeanManager(getSession()).pushNavigatorBeanStack(
                    row.getBigDecimal(ID_UNITA_DOC_NAME), row.getBigDecimal(ID_STRUT_NAME),
                    formList.getName(), formList.getTable(),
    ...
    public void loadDettaglioConListe(String... rigaElemento) throws EMFError, SQLException {
        ...
        AroVLisDocTableBean listDocUd = ricercaBo
                .getAroVLisDocTableBean((BigDecimal) row.getObject(ID_UNITA_DOC_NAME));
        getForm().getDocumentiUDList().setTable(listDocUd);
        ...
        Set<BigDecimal> s = dispenserHelper.findUdsByDlPrefissoValoreString(idStrut,
                codiceIntervento, idUnitaDoc);
        BaseTable bt = null;
        try {
        ...
        NavigatorBeanManager.getNavigatorBeanManager(getSession()).pushNavigatorBeanStack(
                listRow.getBigDecimal(ID_UNITA_DOC_NAME), listRow.getBigDecimal(ID_STRUT_NAME),
    ...
    public BaseRow loadDettaglioConListe(BigDecimal idUnitaDoc, BigDecimal idStrut,
            String codiceIntervento) throws SQLException, EMFError {
    ...
    public void downloadRecupero(String... rigaElemento) throws EMFError {
        ...
        BigDecimal idStrut = DynamicSpagoLiteForm.getRicercaList(getForm()).getTable()
                .getCurrentRow().getBigDecimal(ID_STRUT_NAME);
        RecuperoWSBean strut = pievesestinaBO.getUdData(idStrut,
                row.getBigDecimal(ID_UNITA_DOC_NAME));
