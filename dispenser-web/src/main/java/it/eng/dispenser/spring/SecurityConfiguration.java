...
public class SecurityConfiguration extends ParerSecurityConfiguration {
    ...
    SecurityFilterChain app(HttpSecurity http,
        ...
        http.authorizeHttpRequests(authorize -> authorize.requestMatchers("/logout").permitAll()
                ...
                .requestMatchers("/saml/SingleLogout/alias/" + nomeApplicazione).permitAll().requestMatchers("/rest/**")
                .permitAll().requestMatchers("/saml/**").permitAll().requestMatchers("/saml2/**").permitAll()
                .requestMatchers("/*.html").authenticated().anyRequest().permitAll())
