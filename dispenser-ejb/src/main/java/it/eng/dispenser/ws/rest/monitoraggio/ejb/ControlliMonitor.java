...
public class ControlliMonitor {
	...
	try {
	    String queryStr = "select max(lj.dtEvento) " + "from DipLogJob lj "
		    + "where lj.nmJob = :nmJob " + "and lj.tiEvento = :tiEvento ";
	...
	try {
		...
		if (lstJob != null && lstJob.size() > 0) {
		    ...
		    rispostaControlli.setrObject(
			    ConstDipLogJob.tiEvento.valueOf(lstJob.get(0).getTiEvento()));
		} else {
	...
	try {
	    String queryStr = "select count(j) from DipLogJob j "
		    + "where j.dtEvento >= :dataInizio " + "and j.dtEvento < :ultimaAttivita "
		    + "and j.tiEvento = :tiEvento " + "and j.nmJob = :nmJob ";
