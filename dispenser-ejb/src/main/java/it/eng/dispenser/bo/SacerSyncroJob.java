...
public class SacerSyncroJob {
    ...
    public void sincronizzazioneConSacer() throws Exception {
        ...
        for (BigDecimal structureID : structureIDList) {
            ...
            logger.info("Sincronizzazione con Sacer - IDStruct: {}  data ultima elaborazione: {}.",
                    structureID, dateLastElaboration);
    ...
    public void execute(BigDecimal structureID, Date dateFrom, Date dateTo) throws Exception {
        ...
        logger.info("Processo struttura {} per il range di date {} - {}", structureID, dateFrom,
                dateTo);
        tipiDatoProfiloUd = getTipiDatoProfilo(dateFrom,
                ConstDipTipoDatoProfilo.tiOrigineDatoProfilo.UNI_DOC.name());
        ...
        long numUdEscluse = 0;
        Object[] ogg = getDatiProfiloInserted(structureID, dateFrom, dateTo, tipiDatoProfiloUd,
                numUdEscluse);
    ...
    private Date getDateLastElaboration(BigDecimal structureID) throws Exception {
        ...
        if (list.isEmpty()) {
            logger.warn("Entry non presente nella DIP_SYNCRO_ARO per idStructure = {}",
                    structureID);
            ...
            throw new Exception(
                    "Ci sono due entry nella DIP_SYNCRO_ARO per idStructure = " + structureID);
        }
    }

    // METODI PER METADATI UNITA DOCUMENTARIE
    /**
     * Restituisce i <code>MetadatoUD</code> delle UnitÃƒÂ  Documentarie inserite da
     * <code>dateFrom</code> (inclusa) a <code>dateTo</code> (esclusa) per la
     ...
     * @return
     */
    private List<MetadatoUD> getMetadatiUDInserted(BigDecimal structID, Date dateFrom,
            Date dateTo) {
        // MAC#31007 - Correzione logica di caricamento dati dei versamenti per le dimensioni di
        // prod
    ...
    private void insertMetadatiUD(List<MetadatoUD> metadati) {
        ...
        for (MetadatoUD metadato : metadati) {
            // MEV#30872 - Arricchire il livello di log INFO dei caricamenti effettuati dal
            // JOB_SYNCRO
    ...
    /**
     * Restituisce i <code>MetadatoUD</code> delle Unita Documentarie cancellate da
     * <code>dateFrom</code> (inclusa) a <code>dateTo</code> (esclusa) per la
     * <code>structureID</code>.
     *
     * @param structID
    ...
    private List<MetadatoUD> getMetadatiUDAnnul(BigDecimal structID, Date dateFrom, Date dateTo) {
        String query = "select new  it.eng.dispenser.bean.MetadatoUD(ads.idAttribDatiSpec, ud.idUnitaDoc, vad.dlValore, ads.tiAttribDatiSpec) "
                ...
                + "and stato.tiStatoRichAnnulVers='EVASA' "
                + "and stato.dtRegStatoRichAnnulVers >= :dateFrom "
                + "and stato.dtRegStatoRichAnnulVers < :dateTo "
                + "and da.tiEntitaSacer = 'UNI_DOC' " + "and ads.tiAttribDatiSpec IS NOT NULL "
    ...
    private int deleteMetadatiUD(List<MetadatoUD> metadati) throws Exception {
        ...
        for (MetadatoUD metadato : metadati) {
            ...
            query.append(
                    " d WHERE (d.id.idUnitaDoc = :udID) AND (d.id.idAttribDatiSpec = :attribID)");
    ...
    /**
     * Restituisce i <code>MetadatoDoc</code> delle UnitÃƒÂ  Documentarie inserite da
     * <code>dateFrom</code> (inclusa) a <code>dateTo</code> (esclusa) per la
     * <code>structureID</code>.
     *
     * @param structID
     ...
     * @return
     */
    private List<MetadatoDoc> getMetadatiDocInserted(BigDecimal structID, Date dateFrom,
            Date dateTo) {
        // MAC#31007 - Correzione logica di caricamento dati dei versamenti per le dimensioni di
        // prod
        String query = "SELECT NEW  it.eng.dispenser.bean.MetadatoDoc(ads.idAttribDatiSpec, doc.idDoc, ud.idUnitaDoc, vad.dlValore, ads.tiAttribDatiSpec) "
                + "FROM AroUnitaDoc ud, VrsSessioneVers ses " + "JOIN ud.aroUsoXsdDatiSpecs da "
                + "JOIN da.aroValoreAttribDatiSpecs vad " + "JOIN vad.decAttribDatiSpec ads "
                + "JOIN da.aroDoc doc " + "WHERE (ses.idStrut = :structureID) "
                ...
                + "AND ud.idUnitaDoc=ses.idUnitaDoc " + "AND ses.tiSessioneVers='VERSAMENTO' "
                + "AND ses.tiStatoSessioneVers='CHIUSA_OK' ";
    ...
    /**
     * Restituisce i <code>MetadatoDoc</code> delle Unita Documentarie cancellate da
     * <code>dateFrom</code> (inclusa) a <code>dateTo</code> (esclusa) per la
     * <code>structureID</code>.
     *
     * @param structID
    ...
    private List<MetadatoDoc> getMetadatiDocAnnul(BigDecimal structID, Date dateFrom, Date dateTo) {
        ...
        String query = "select NEW it.eng.dispenser.bean.MetadatoDoc(ads.idAttribDatiSpec, doc.idDoc, ud.idUnitaDoc, vad.dlValore, ads.tiAttribDatiSpec) "
                ...
                + "where rich.orgStrut.idStrut = :structureID "
                + "and stato.tiStatoRichAnnulVers = 'EVASA' "
    ...
    /*
     * Torna l'elenco dei dati di profilo di tipo documento principale o Unita doc e con data
     * istituz. maggiore uguale a quella passata.
     */
    ...
    /**
     * Restituisce i<code>DatoProfilo</code> delle UnitÃ Â  Documentarie inseritele informazioni di
     * profilazione delle Unita Documentarie inserite da <code>dateFrom</code> (inclusa) a
     * <code>dateTo</code> (esclusa) per la <code>structureID</code>. Il contatore deve essere
     * tornato al chiamante perchÃ© viene incrementato se alcune ud vengono escluse dal conteggio
     * dei dati di profilo nel caso di Strumenti urbanistici
     *
     * @param structID
     * @param dateFrom
     * @param dateTo
    ...
    private Object[] getDatiProfiloInserted(BigDecimal structID, Date dateFrom, Date dateTo,
        ...
        /*
         * Deve determinare se si sta lavorando su una struttura che fa capo ad una struttura
         * fittizia (caso PUG)
         */
        ...
        // MAC#31007 - Correzione logica di caricamento dati dei versamenti per le dimensioni di
        // prod
        String query = "SELECT ud FROM AroUnitaDoc ud, VrsSessioneVers ses "
                + "WHERE (ses.idStrut = :structureID) " + "AND (ses.dtChiusura >= :dateFrom "
                + "AND ses.dtChiusura < :dateTo) " + "AND ud.orgStrut.idStrut=ses.idStrut "
                + "AND ud.idUnitaDoc=ses.idUnitaDoc "
                // MAC#31066 - correzione estrazione dati UD in caso di aggiunta documenti
                + "AND ses.tiSessioneVers='VERSAMENTO' "
                + "AND ses.tiStatoSessioneVers='CHIUSA_OK' ";
        Query q = em.createQuery(query);
        ...
        for (AroUnitaDoc uniDoc : list) {
            /*
             * Nel caso in cui si sta lavorando per una struttura che fa campo ad una fittizia (caso
             * PUG) deve verificare se la UD reale estratta esiste anche nella struttura fittizia.
             * Se non esiste la esclude dal calcolo
             */
            DecTipoUnitaDoc decTipoUnitaDocFittizio = null;
            DecTipoUnitaDoc decTipoUnitaDocReale = null;
            if (isStrumentoUrbanistico) {
                decTipoUnitaDocReale = em.find(DecTipoUnitaDoc.class,
                        uniDoc.getIdTipoUnitaDoc().longValueExact());
                decTipoUnitaDocFittizio = dispenserHelper.getDecTipoUnitaDocByStrutAndNome(
                        idStruturaFittizia, decTipoUnitaDocReale.getNmTipoUnitaDoc());
            ...
            if (mappaMetodi.size() == 0) {
                ...
                for (DipTipoDatoProfilo tipo : tipiDatoProfilo) {
                    ...
                    try {
                        /*
                         * Nel caso di tipo conversione ENTE_STRUTTURA o TIPO_UD il metodo dinamico
                         * non viene utilizzato altrimenti si generava il Warning di mancato
                         * agganciodel metodo. MAC#27609 - DIPS - indagine sul mancato aggiornamento
                         * di alcune ricerche
                         */
                        if (isStrumentoUrbanistico && tipo.getTipoConversione() != null
                                && (tipo.getTipoConversione()
                                        ...
                                        || tipo.getTipoConversione().equals(
                                                DipTipoDatoProfilo.TipoConversione.TIPO_UD))) {
                            ...
                            logger.debug(
                                    "Agganciato il metodo {} per il dato di profilo configurato.",
                                    nomeMetodo);
                        }
                        mappaMetodi.put(tipo.getNmColonnaDb(), metodo);
                    } catch (NoSuchMethodException | SecurityException ex) {
                        logger.warn("Errore nell'aggancio del metodo {} della classe AroUnitaDoc",
                                nomeMetodo, ex);
                    }
                }
            }
            ...
            for (DipTipoDatoProfilo tipo : tipiDatoProfilo) {
                try {
                    // MODIFICA PER PUG!
                    /*
                     * Se tipo conversione (DEL TIPO DATO PROFILO) Ã¨ uguale a "TIPO_UD" si prende
                     * il nome del tipo UD reale e l'id struttura reale, si cerca a quale struttura
                     * fittizia appartiene e poi con l'id della struttura fittizia ed il nome tipo
                     * ud estratto si cerca il tipo ud associato alla struttura fittizia. L'ID del
                     * tipo UD fittizio trovato viene messo nel "valore" del DatoProfilo
                     */
                    if (isStrumentoUrbanistico && tipo.getTipoConversione() != null
                            && tipo.getTipoConversione()
                                    .equals(DipTipoDatoProfilo.TipoConversione.TIPO_UD)) {
                        // MAC#19891 - Problema al job null pointer exception. Possono esserci UD
                        // non presenti nella
                        // struttura fittizia, da bypassare
                        datoProfilo = new DatoProfilo(structID, tipo.getNmTipoDatoProfilo(),
                                uniDoc.getIdUnitaDoc(),
                                new BigDecimal(decTipoUnitaDocFittizio.getIdTipoUnitaDoc()),
                                tipo.getTiDatoProfilo(), uniDoc.getIdTipoUnitaDoc());
                        res.add(datoProfilo);
                        ...
                        datoProfilo = new DatoProfilo(structID, tipo.getNmTipoDatoProfilo(),
                                uniDoc.getIdUnitaDoc(),
                                new BigDecimal(uniDoc.getOrgStrut().getIdStrut()),
                                tipo.getTiDatoProfilo(), uniDoc.getIdTipoUnitaDoc());
                        res.add(datoProfilo);
                        ...
                        if (valore != null) {
                            datoProfilo = new DatoProfilo(structID, tipo.getNmTipoDatoProfilo(),
                                    uniDoc.getIdUnitaDoc(), valore, tipo.getTiDatoProfilo(),
    ...
    private List<DatoProfilo> getDatiProfiloAnnul(BigDecimal structID, Date dateFrom, Date dateTo,
        ...
        String query = "select ud " + "from AroRichAnnulVer rich "
                + "join rich.aroStatoRichAnnulVersCor stato "
                + "join rich.aroItemRichAnnulVers item " + "join item.aroUnitaDoc ud "
                + "where rich.orgStrut.idStrut =:structureID "
                + "and stato.dtRegStatoRichAnnulVers >= :dateFrom "
                + "and stato.dtRegStatoRichAnnulVers < :dateTo "
                + "and item.tiItemRichAnnulVers = 'UNI_DOC' "
                + "and item.tiStatoItem = 'ANNULLATO' ";
        Query q = em.createQuery(query);
        q.setParameter(STRUCTURE_ID, structID.longValue());
        q.setParameter(DATE_FROM, dateFrom);
     ...
     * @param structureID
     * @param date
     * @param udInserted  the number of UD inserted in the transaction
     * @param udDeleted   the number of UD annul in the transaction
     *
     * @throws Exception
     */
    private void updateLastElaboration(BigDecimal structureID, Date dateFrom, Date dateTo,
            long numUdEscluse) throws Exception {
        long udInserted;
        long udDeleted;
        long docInserted;
        long docDeleted;
        ...
        Query q = em
                .createQuery("SELECT d " + "FROM DipSyncroAro d " + "WHERE d.idStrut = :structure");
        ...
        BigDecimal udSinc = BigDecimal
                .valueOf(element.getNiUdSincro().doubleValue() + udInserted - numUdEscluse);
        ...
        BigDecimal docSinc = BigDecimal
                .valueOf(element.getNiDocSincro().doubleValue() + docInserted);
    ...
    private long getNumUDInserted(BigDecimal structID, Date dateFrom, Date dateTo) {
        // MAC#31007 - Correzione logica di caricamento dati dei versamenti per le dimensioni di
        // prod
        String query = "SELECT COUNT(DISTINCT ud.idUnitaDoc) "
                + "FROM AroUnitaDoc ud, VrsSessioneVers ses " + "WHERE ses.idStrut = :structureID "
                + "AND (ses.dtChiusura >= :dateFrom " + "AND ses.dtChiusura < :dateTo) "
                + "AND ud.orgStrut.idStrut=ses.idStrut " + "AND ses.idUnitaDoc=ud.idUnitaDoc "
                // MAC#31066 - correzione estrazione dati UD in caso di aggiunta documenti
                + "AND ses.tiSessioneVers='VERSAMENTO' "
                + "AND ses.tiStatoSessioneVers='CHIUSA_OK' ";
        Query q = em.createQuery(query);
    ...
    private long getNumDocInserted(BigDecimal structID, Date dateFrom, Date dateTo) {
        // MAC#31007 - Correzione logica di caricamento dati dei versamenti per le dimensioni di
        // prod
        String query = "SELECT COUNT(DISTINCT doc.idDoc) "
                + "FROM AroUnitaDoc ud, VrsSessioneVers ses " + "JOIN ud.aroUsoXsdDatiSpecs da "
                + "JOIN da.aroValoreAttribDatiSpecs vad " + "JOIN vad.decAttribDatiSpec ads "
                + "JOIN da.aroDoc doc " + "WHERE (ses.idStrut = :structureID) "
                + "AND (ses.dtChiusura >= :dateFrom " + "AND ses.dtChiusura < :dateTo) "
    ...
    /*
     * Dato un nome di colonna in formato ORACLE (database es.: CAMPO_PROVA) torna getCampoProva
     * come nome di metodo.
     */
